...
class TaskBlogApp {
        ...
        frappe.db.get_list('Projects', {
            ...
            switch (this.currentFilter) {
                case 'today':
                    ...
                    break;
                case 'high':
                    filteredProjects = self.projects.filter(task => task.priority === 'high');
                    break;
                case 'completed':
                    filteredProjects = self.projects.filter(task => task.status === 'completed');
                    break;
                case 'pending':
                    filteredProjects = self.projects.filter(task => task.status === 'pending');
                    break;
            }
                ...
                <div class="project-post" data-project-id=${p.name}>
                    <div class="project-post-header">
                        ...
                        <div class="project-post-meta">
                            <div class="task-author-name">${p.user}</div>
                            <div class="project-post-date">${this.formatDate(p.creation)}</div>
                        </div>
                        <div class="project-status ${p.status.replace('-', '')}">${p.status.replace('-', ' ').toUpperCase()}</div>
                    </div>
                    <h2 class="project-title">${p.project_name} <span class="project-category">${p.category.toUpperCase()}</span></h2>
    ...
    renderTasks(project) {
        ...
        frappe.db.get_list('Tasks', {
            ...
            blogPosts.innerHTML = t.map(post => `
                <div class="blog-post">
    ...
    openProject(){
        ...
        $(document).off("click",".project-post").on("click",".project-post",function(event){
            ...
            let container_content = `
                ...
                <button class="add-task-btn" id="open-task-modal">+</button>
    ...
    createTask() {
        $(document).on("click", "#open-task-modal", function (event) {
            ...
            let d = new frappe.ui.Dialog({
                title: "Custom Dialog",
                fields: [
                    { fieldtype: 'Data', label: 'To', fieldname: 'to' },
                    { fieldtype: 'Data', label: 'Subject', fieldname: 'subject' },
                    { fieldtype: 'Text Editor', label: 'Message', fieldname: 'message' }
                ],
                primary_action_label: 'Send',
                primary_action(values) {
                    frappe.msgprint(`Sending to ${values.to}...`);
                    d.hide();
                }
            });
            d.show();
    ...
    deleteTask() {
        if (!this.currentTaskId) return;
        this.projects = this.projects.filter(task => task.id !== this.currentTaskId);
        this.saveProjects();
        // this.renderTasks();
        this.renderProjects()
        this.updateStats();
    ...
    addProject() {
        ...
        frappe.call({
            method:"task_blogger.task_blogger.page.task_blogging.task_blogging.new_project",
            args:{
                user:frappe.session.user,
        ...
        this.renderProjects()
        this.updateStats();
        this.closeModal(); // idont know its working or not but after create button trigger then module not close automatically
        form.reset();
        this.setDefaultDateTime();
    ...
    updateStats() {
        ...
        $('#totalTasks').text(this.projects.length);
